...
{
	TSharedPtr<FARSystemBase, ESPMode::ThreadSafe> ARSystem = GetARSystem();
	if (RegionLocalToTrackingTransforms.Contains(Region))
	{
		return RegionLocalToTrackingTransforms[Region] * ARSystem->GetAlignmentTransform() * ARSystem->GetTrackingToWorldTransform();
	}
